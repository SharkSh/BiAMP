...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps):
    ...
    返回一个长度为 num_diffusion_timesteps 的 NumPy 数组，包含每个时间步的 β 值。
    """
    if schedule_name == "linear":
        ...
        return betas_for_alpha_bar(
            num_diffusion_timesteps,
            lambda t: 1-np.sqrt(t + 0.0001), # alpha_bar
        )
...
class GaussianDiffusion:
    ...
    def __init__(
        self,
        *,
        betas,                             # betas控制噪声增加的速率
        model_mean_type,                   # 模型输出的均值类型
        model_var_type,                    # 模型输出的方差类型
        loss_type,                         # 损失函数类型
        rescale_timesteps=False,           # 是否对时间步进行归一化
        model_arch=None,                   # 模型架构
    ...
    # 计算扩散模型中后验分布 q(x_{t-1} | x_t, x_0) 的均值和方差
    ...
    def ddim_sample(
        ...
        n for pick initiate value from grid for z (noise)
        """
        out = self.p_mean_variance(
            model,
            x,
        ...
        eps = self._predict_eps_from_xstart(x, t, out["pred_xstart"])   # 用x0推出噪声
        ...
        if langevin_fn:
            print('t.shape in langevin_fn: ', t.shape)
    ...
    def ddim_sample_loop_progressive(
        ...
        m for pick initiate value from grid for x_t (img)
        n for pick initiate value from grid for z (noise)
        """
        if device is None:
            device = next(model.parameters()).device
        ...
        if noise is not None:
            ...
            img = (-m - m) * th.rand(shape, device=device) + m
            img_init = img
        else:
            img = th.randn(*shape, device=device)
            img_init = img
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                out["init"] = img_init
                if i % 10 == 0: # save img every 10 steps
                    progress_img = th.cat((progress_img, out["sample"].unsqueeze(0)),
                                          dim=0)
                out["progress"] = progress_img
                yield out
                img = out["sample"]
