...
def langevin_fn3_mod(debug_lst, model_control, label_ids, step_size, sample, mean, sigma,
                 alpha, t, prev_sample):  # current best.
    if t[0].item() < 10:
        K = 0
    else:
    ...
    with th.enable_grad():
        for i in range(K):
            ...
            model_out = model_control(input_embs)
            ...
            model_loss = loss_fn(model_out, label_ids2.type(th.FloatTensor).cuda())
            
            loss = model_loss + logp_term
            loss.backward()
            optimizer.step()
            epsilon = th.randn_like(input_embs_param.data)
            input_embs_param = th.nn.Parameter((input_embs_param.data + 0.0 * sigma.mean().item() * epsilon).detach())
...
def langevin_fn3_nocontrol(debug_lst, model_control, label_ids, step_size, sample, mean, sigma,
                 alpha, t, prev_sample):  # current best.
    if t[0].item() < 10:
        K = 0
    else:
