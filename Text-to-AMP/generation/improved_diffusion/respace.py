...
class _WrappedModel:
    ...
    def __call__(self, x, ts, **kwargs):
        ...
        text_emb = text_emb.to(dtype=new_ts.dtype)
        length = kwargs.pop("length", None)
        if length is not None:
            if not isinstance(length, th.Tensor):
                length = th.tensor(length, dtype=new_ts.dtype, device=new_ts.device)
            else:
                length = length.to(dtype=new_ts.dtype, device=new_ts.device)
        return self.model(x, text_emb, length, new_ts, **kwargs)
